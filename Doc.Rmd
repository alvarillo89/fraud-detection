...
- `VIM`: para la imputación de valores perdidos.
...
- `DMwR`: para generar ejemplos con el método SMOTE.
...
En este apartado se describen aquellos algoritmos de clasificación que se aplicarán para intentar predecir si una transacción online es o no fraudulenta. Para que la comparación entre los preprocesamientos realizados sea justa, los modelos empleados en todas las pruebas son los mismos. También se mantienen los mismos hiperparámetros, los cuales han sido fijados con el conjunto datos preprocesado del [experimento 1](#experimento-1): 
...
# Histograma con escala logrítmica en ambos ejes:
ggplot(data.raw) + 
  geom_histogram(aes(x=TransactionAmt, fill=as.factor(isFraud))) +
...
Este fichero incluye 40 características nuevas a las 393 ya existentes (si no se tiene en cuenta el id). Por otro lado, reduce el número de filas, pues solo 144.233 ejemplos tienen informacion de identidad. Si ambos conjuntos de datos se uniesen, se eliminarían 446.307 observaciones, por lo que puede resultar útil para reducir la dimensionalidad.
...
3. Validación de los modelos. Se evaluan y comparan los modelos entrenados en la fase anterior calculando la curva ROC, el área bajo la curva ROC, sensitivity, specificity y Kappa. Estas métricas serán las que se utilicen para elegir el mejor preprocesamiento de todos los realizados y también para elegir el mejor modelo entrenado.
...
A continuación se describe brevemente el preprocesamiento realizado en este primer experimento, en el que solo se ha utilizado el dataset `transaction` (no se han incluido las columnas del dataset `identity`):
...
- Imputar valores perdidos para la clase 1:
  ...
  - Las variable numéricas, son imputadas con la media de cada columna. Posiblemente este no sea el método más sofisticado, pero ha sido el único abarcable computacionalmente. Previamente a este decisión se intentó utilizar el paquete `MICE` para la imputación, pero no llegué a obtener ningún resultado (el ordenador se quedaba colgado por demasiada carga de trabajo). 
...
as.data.frame(df_status(test.full, F))
```

Puede verse que, al igual que el conjunto de train, presenta una gran cantidad de valores perdidos. Sin embargo, en este caso no podemos eliminar filas. Habrá que imputar todos los valores.

Antes que nada, renombramos algunas de las columnas de test que no tienen el mismo nombre que en train:

```{r}
colnames(test.full) <- gsub("id.", "id_", colnames(test.full))
```

Ahora sí, aplicamos el mismo preprocesamiento que se aplicó a los datos de train del [experimento 2](#experimento-2). 
...
Se obtiene un **AUC de 0.786960**. Como era de esperar se obtienen resultados mejores que con el anterior, pero tampoco es una mejora remarcable. Esto me hace pensar que el problema de los modelos no es el sobreajuste, sino los NA. En train se eliminaban aquellas columnas con más de un 50% de valores perdidos porque imputarlas no daría buenos resultados. En este caso, no ha sido posible eliminarlas y, como era de esperar, la imputación ha afectado negativamente a la calidad de los resultados.

---

# Conclusión

En todos los experimentos, los datos, modelos e hiperparámetros han sido los mismos. Sin embargo, los resultados obtenidos han sido muy diferentes. Esto pone de manifiesto la importancia de realizar un buen preprocesamiento de los datos, y su influencia sobre las métricas finales. 

Cuando se afronta un nuevo promblema de aprendizaje automático, se le suele prestar una especial atención al ajuste de los modelos (qué clase de funciones utilizar, cómo fijar los hiperparámetros, que métricas se emplearán para evaluar, si se realizará o no validación cruzada, etcétera). Es cierto que todo eso es importante, pero no se debe olvidar que un modelo de aprendizaje automático no es nada sin datos, y esos datos deben tener la calidad suficiente. Esta práctica demuestra que en el mundo real los datos suelen tener una calidad extremadamente pobre, y que si se desea construir un buen clasificador que ayude a resolver un problema, es de vital importancia dedicarle tiempo suficiente al estudio, preparado y preprocesamiento de los conjuntos de datos. Es un proceso que no puede desligarse de los sistemas inteligentes y que requiere tanto o más atención que el resto.

Además, la practica también demuestra que muchas veces la capacidad computacional es una traba para resolver problemas de esta índole. Procesar grandes cantidades de datos require de una gran capacidad de cómputo, capacidad que la que no siempre se dispone, limitando en ocasiones las técnicas que pueden aplicarse. En esos casos hay que usar el ingenio y buscar alternativas menos costosas que permitan avanzar en la resolución del problema: emplear técnicas menos sofisticadas pero más eficientes, reducir la dimensionalidad, trabajar con submuestras, etcétera. En la mayoría de los casos se deberá sacrificar información, pero la clave está en encontrar un equilibrio: sacrificar la información suficiente como para que se pueda resolver el problema con los recursos disponibles.
